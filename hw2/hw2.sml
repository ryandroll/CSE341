...
fun all_except_option (keyword, slst) =
    let 
        val answer = List.filter (fn x => not (same_string (x, keyword))) slst
    in 
        if answer = slst then NONE 
        else SOME answer
    end
...
fun officiate (cl : card list, ml : move list, goal) =
    let
        fun tail_helper (cl : card list , hl : card list , ml : move list) =
            case (cl, hl, ml)
             of (_, _, []) => hl
              | (_, _, Discard m :: ms') => tail_helper(cl,
                                                       remove_card(hl, m, IllegalMove),
                                                       ms')
              | (_, [],  Draw :: ms') => hl
              | (_, c :: cs',  Draw :: ms') => if sum_cards (c :: hl) > goal
                                              then c :: hl
